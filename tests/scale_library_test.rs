...
fn test_metadata_has_interpretation() {
    for scale_name in scales::list_available_scales() {
        ...
        assert!(!metadata.interpretation.is_empty(), 
            "{} should have interpretation", scale_name);
    }
}
...
fn test_metadata_has_citation() {
    for scale_name in scales::list_available_scales() {
        ...
        assert!(!metadata.citation.is_empty(), 
            "{} should have citation", scale_name);
    }
}
...
fn test_all_configs_are_valid_toml() {
    for scale_name in scales::list_available_scales() {
        ...
        assert!(parse_result.is_ok(), 
            "{} config should be valid TOML: {:?}", scale_name, parse_result);
    }
}
...
fn test_configs_have_required_sections() {
    for scale_name in scales::list_available_scales() {
        ...
        assert!(config.contains("[survey]"), 
            "{} should have [survey] section", scale_name);
        assert!(config.contains("[quality]"), 
            "{} should have [quality] section", scale_name);
        assert!(config.contains("[output]") || config.contains("decimal_places"), 
            "{} should have output configuration", scale_name);
        assert!(config.contains("[scales."), 
            "{} should have at least one scale definition", scale_name);
    }
}
